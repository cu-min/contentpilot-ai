...
敏感信息必须仅保存在后端本地配置中，前端脱敏展示，日志不明文输出，Git 不提交 Cookie、Token、API Key 或浏览器用户目录。平台账号 `auth_config` 使用 AES-256-GCM 加密；生产环境必须配置 Base64 编码的 32 字节 `PLATFORM_CREDENTIAL_KEY`，并稳定保管该密钥。

旧明文配置可兼容读取；配置密钥后，新建或更新账号会保存为 `enc:v1:` 密文，旧账号在下一次更新时转密文。系统不自动批量迁移，升级后应逐个保存现有账号并检查迁移结果。密钥丢失或随意更换会导致已有密文无法解密。
