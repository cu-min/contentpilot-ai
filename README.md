...
- 🔐 自动发布涉及平台登录态、Cookie、验证码和风控，必须保留人工确认边界。
...
当前 MVP 优先验证发布任务流转和安全边界。部分真实平台发布能力仍采用人工确认或草稿更新方式。
...
提交后，任务进入 `PENDING` 状态。运营人员可以手动执行。对于掘金草稿试点，系统会更新已有草稿，然后由人进入掘金编辑器做最后确认。
