...
ContentPilot AI 是一个面向内容运营、增长团队和产品营销人员的内部工具。它的目标不是简单生成一篇文章，而是把一篇营销内容从 **产品资料整理、AI 初稿生成、多平台改写、发布任务创建、发布结果记录** 串成一条完整工作流。
...
当前 MVP 已打通掘金真实发布链路：系统会自动新建草稿、更新内容并提交发布；其他平台 Publisher 后续逐步接入。
...
提交后，任务进入 `PENDING` 状态。运营人员触发自动发布后，系统会为掘金新建草稿、写入平台稿内容并提交正式发布，成功后记录文章链接。
...
掘金平台目前支持新建草稿、更新内容并提交发布。涉及 Cookie、csrfToken、categoryId、tagIds 等本地配置时，请只保存在平台账号的本地认证配置中，不要提交到仓库。
