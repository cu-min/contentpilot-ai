...
| 平台 | 当前发布能力 |
|---|---|
| 微信公众号 | 官方 API 创建草稿或提交发布，并记录本次执行结果 |
| 掘金 | 非官方接口试点：新建草稿、更新内容、提交发布并记录结果链接 |
...
点击提交时选择“立即发布”或“定时发布”。两类任务都会进入 `PENDING`，由运营人员在计划时间到达后手动点击执行；当前 MVP 不扫描或自动执行定时任务。系统会根据平台账号的默认发布方式选择 Publisher：掘金走非官方接口试点，微信公众号走官方 API，CSDN 和知乎走 Chrome for Testing 浏览器自动化。成功后记录发布链接，失败则记录可读原因。
...
- 发布任务创建、提交、取消、执行
- 发布任务执行链路验证
- 微信公众号官方 API 草稿创建、可选正式发布与结果记录
- 掘金新建草稿、更新内容并提交真实文章发布
- CSDN 浏览器自动化填充和自动发布试点
- 知乎浏览器自动化填充、发布设置处理和自动发布试点
- 统一发布任务状态：`DRAFT`、`PENDING`、`RUNNING`、`SUCCESS`、`FAILED`、`CANCELLED`
